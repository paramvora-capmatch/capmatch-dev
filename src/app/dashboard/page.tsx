...
export default function DashboardPage() {
	...
	const new_user_profile_completion_threshold = 20;
	...
	useEffect(() => {
		...
		// After initial load is complete, check for new user profile and redirect if necessary.
		if (
			initialLoadComplete &&
			user?.role === "borrower" &&
			borrowerProfile &&
			projects.length === 0 // A key indicator of a new user who hasn't done anything
		) {
			// Define a "new user" as someone with an incomplete profile and no projects.
			if (
				borrowerProfile.completenessPercent <
				new_user_profile_completion_threshold
			) {
				console.log(
					"New user detected (incomplete profile, no projects), redirecting to profile page."
				);
		...
		// After initial load is complete, check for new user profile
		if (
			initialLoadComplete &&
			user?.role === "borrower" &&
			borrowerProfile
		) {
			// Define a "new user" as someone with a very incomplete profile.
			if (
				borrowerProfile.completenessPercent <
					new_user_profile_completion_threshold &&
				autoCreatedFirstProjectThisSession
						...
						<div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
							<div className="space-y-1">
								...
								<p className="text-gray-600">
									Manage and track your commercial real estate
									deals
								</p>
							</div>
							...
							<div className="relative">
								...
								<div className="relative text-center py-16 bg-white/70 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-100">
									...
									<p className="text-gray-600 mb-8 max-w-md mx-auto">
										Ready to find the perfect lenders for
										your commercial real estate deal? Start
										by exploring our curated lender
										marketplace.
									</p>
