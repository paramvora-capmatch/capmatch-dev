...
export default function SitePlanPage() {
	...
	const buildingFootprint = content?.grossBuildingArea
		? `${content.grossBuildingArea} SF`
		: null;
	const parkingSpaces = content?.parkingSpaces ?? null;
	const greenSpace = content?.greenSpace
		? `${formatLocale(parseNumeric(content.greenSpace))} SF`
		: null;
	...
	const heightLimit = parseNumeric(content?.heightLimit) ?? null;
	const actualHeight = parseNumeric(content?.actualHeight) ?? null;

	// Setbacks from content
	const setbacks = {
		front: content?.setbackFront ?? null,
		side: content?.setbackSide ?? null,
		rear: content?.setbackRear ?? null,
	};
	...
	const zoningDetails = {
		...
		heightLimit: heightLimit != null ? `${heightLimit} ft` : null,
		actualHeight: actualHeight != null ? `${actualHeight} ft` : null,
	};
	...
	const heightRemaining =
		heightLimit != null && actualHeight != null
	...
	useOMPageHeader({
		subtitle:
			"Layout of buildings, circulation, zoning, and site efficiencies.",
	});
		...
		<div className="space-y-6">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
				<Card className="hover:shadow-lg transition-shadow">
					<CardHeader
						className="pb-2"
						dataSourceFields={["total site acreage"]}
					>
				...
				<Card className="hover:shadow-lg transition-shadow">
					<CardHeader
						className="pb-2"
						dataSourceFields={["parking spaces"]}
					>
						<div className="flex items-center">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
						...
						<div className="space-y-3">
							<div>
								...
								<p className="font-semibold text-gray-800">
									{zoningDetails.heightLimit ?? null}
								</p>
							</div>
							<div>
								<p className="text-sm text-gray-500">
								...
								<p className="font-semibold text-gray-800">
									{zoningDetails.actualHeight ?? null}
								</p>
							</div>
							<div>
								<p className="text-sm text-gray-500">
							...
							<div>
								...
								<div className="space-y-1">
									<p className="text-xs text-gray-600">
										Front: {setbacks?.front ?? null}
									</p>
									<p className="text-xs text-gray-600">
										Side: {setbacks?.side ?? null}
									</p>
									<p className="text-xs text-gray-600">
										Rear: {setbacks?.rear ?? null}
									</p>
								</div>
							</div>
						</div>
					...
					<CardContent>
						<div className="space-y-4">
							...
							<div className="flex justify-between items-center">
								...
								<Badge variant="secondary">
									{content?.greenSpaceRatio != null
										? `${content.greenSpaceRatio}%`
										: null}
								</Badge>
							</div>
						</div>
