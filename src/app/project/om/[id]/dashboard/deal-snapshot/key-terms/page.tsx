...
export default function KeyTermsPage() {
	...
	const interestOnlyPeriodMonths =
		parseNumeric(content?.interestOnlyPeriodMonths) ?? null;
	...
	const keyTerms = {
		...
		exitFee: content?.exitFee ?? content?.prepaymentPremium ?? null,
		interestRateType,
		...
		covenants: {
			minDSCR:
				content?.dscrStressMin != null
					? `${content.dscrStressMin.toFixed(2)}x`
					: null,
			maxLTV:
			...
			minLiquidity:
				content?.guarantorLiquidity != null
					...
					: null,
			completionGuaranty: content?.completionGuaranty ?? null,
		},
		lenderReserves: {
			interest:
			...
			taxInsurance: (() => {
				...
				if (realEstateTaxes != null && insurance != null) {
					return `$${Number(
			...
			capEx:
				content?.capExReserve != null
					? `$${Number(content.capExReserve).toLocaleString()}`
					: null,
		},
	};
	...
	if (content?.opportunityZone === true) {
		specialPrograms.push({
			name: "Opportunity Zone",
			description: "Qualified Opportunity Zone benefits",
		});
	}
	if (content?.taxExemption === true) {
		specialPrograms.push({
			name: "Tax Exemption",
			description:
				content?.exemptionStructure || "Tax exemption structure",
		});
	}
	if (content?.affordableHousing === true) {
		specialPrograms.push({
			...
			description: `${
				content?.affordableUnitsNumber || 0
			} affordable units`,
		});
	}
	...
	useOMPageHeader({
		subtitle:
			"Quick read on rate, term, recourse, and covenant highlights.",
	});
		...
		<div className="space-y-6">
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
				...
				<Card className="hover:shadow-lg transition-shadow border-blue-200 bg-white">
					<CardHeader
						className="pb-3"
						dataSourceSection="lender reserves"
					>
						<div className="flex items-center space-x-2">
			...
			<Card className="hover:shadow-lg transition-shadow border-blue-200 bg-white">
				<CardHeader
					className="pb-3"
					dataSourceSection="financial covenants"
				>
					<div className="flex items-center space-x-2">
						<FileText className="h-6 w-6 text-blue-600" />
						<h3 className="text-xl font-semibold text-gray-800">
