/**
 * GET /api/matchmaking/latest?projectId=...
 * Proxies to FastAPI backend to get latest matchmaking results.
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const rl = checkRateLimit(rlId, GENERAL_RATE_LIMIT, "matchmaking-latest");
    ...
    const res = await fetch(`${backendUrl}/api/v1/matchmaking/${projectId}/latest`, {
      method: "GET",
      headers: {
    ...
    const message = data?.detail ?? data?.message ?? data?.error ?? "Failed to fetch results";
