...
const getFieldValue = (project: ProjectProfile, fieldId: string): any => {
	...
	if (
		...
		(project as any).content[fieldId] !== undefined
	) {
...
const formatFieldValue = (value: any, dataType?: string): string => {
		...
		if (
			...
			if (typeof extractedValue === "number") {
				switch (dataType) {
					...
					case "Decimal":
						return formatDecimal(extractedValue);
					case "Integer":
	...
	switch (dataType) {
		...
		case "Decimal":
			return typeof value === "number" ? formatDecimal(value) : "N/A";
		case "Integer":
							...
							>
								{(
									...
									const allFieldMetas = allFieldIds
										.map(
											(id) =>
												projectResumeFieldMetadata[id]
										)
										.filter(
											(
												m
											): m is (typeof projectResumeFieldMetadata)[string] =>
												!!m
										);

									// Check if section has any values (fields or special tables)
									const hasAnyValueInSection =
										allFieldMetas.some((field) => {
									...
									const hasRentComps =
										...
										Array.isArray(
											getFieldValue(project, "rentComps")
										) &&
										(
											getFieldValue(
												project,
									...
									const hasValuesInSubsections =
										Array.isArray(step.subsections) &&
										step.subsections.some(
											(subsection: any) => {
												const subsectionFieldIds: string[] =
													subsection.fields || [];
												return subsectionFieldIds.some(
													(fieldId: string) => {
