...
const GraphicContainer: React.FC<{ children: React.ReactNode }> = ({
	children,
}) => {
	return (
			...
			{nodes.map((node, index) => (
				<React.Fragment key={`lender-node-${index}`}>
					...
					<AnimatedConnectionLine
						...
						endY={centerY}
						color={
							node.color === COLORS.MEDIUM_GREY ||
							node.color === COLORS.DARK_GREY
								? COLORS.LIGHT_GREY
								: node.color === COLORS.PRIMARY_GREEN ||
								  node.color === COLORS.LIGHT_GREEN
	...
	const nodes = [
		{
			...
			y: 100,
			Icon: FileText,
			color: COLORS.MEDIUM_GREY, // base grey
			glowColor: COLORS.LIGHT_GREY,
			label: "Raw Data",
			delay: 0.1,
		},
		...
		},
		{
			...
			y: 300,
			Icon: FileJson,
			color: COLORS.DARK_GREY, // slightly darker grey
			glowColor: COLORS.LIGHT_GREY,
			label: "Parsed Docs",
			delay: 0.3,
		},
...
export const ProcessGraphics: React.FC<{ activeIndex: number }> = ({
	activeIndex,
}) => {
	const graphics = [
