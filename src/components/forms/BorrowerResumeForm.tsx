...
export const BorrowerResumeForm: React.FC<BorrowerResumeFormProps> = ({ onComplete }) => {
  ...
  const handleInputChange = useCallback((field: keyof BorrowerResumeContent, value: any) => { setFormData(prev => ({ ...prev, [field]: value })); }, []);
  const handlePrincipalInputChange = useCallback((field: keyof Principal, value: any) => { setPrincipalFormData(prev => ({ ...prev, [field]: value })); }, []);
  ...
  ], [formData, principals, principalFormData, isAddingPrincipal, formSaved, handleInputChange, handleProfileSubmit, handleAddPrincipal, handleRemovePrincipal, handlePrincipalInputChange, borrowerResume?.fullLegalName])
