...
export const NotificationSettingsPanel = () => {
	...
	const fetchProjects = useCallback(async () => {
		...
		if (projectError) {
			console.error(
				"[NotificationSettingsPanel] Failed to load projects:",
				projectError
			);
		...
		<div className="space-y-8">
				...
				<div className="flex items-center justify-between flex-wrap gap-3">
					<div>
						...
						<p className="text-xs text-gray-500">
							Mute all notifications for specific projects.
						</p>
					</div>
				...
				{projectsLoading ? (
					...
					<div className="space-y-3">
						...
						{filteredProjects.length > 6 && (
							<p className="text-xs text-gray-500">
								Showing the first 6 projects. Use search to find
								others.
							</p>
						)}
					</div>
							...
							const label =
								pref.scope_type === "global"
									? pref.event_type === "*"
										? "All notifications"
										: pref.event_type
									: projectNameMap[pref.scope_id ?? ""] ??
									  pref.scope_id?.slice(0, 6) ??
									  pref.scope_type;
							return (
								<span
			...
			{!isLoading && preferences.length === 0 && (
				<div className="flex items-center gap-2 text-xs text-gray-500">
					...
					No overrides yet — you’re receiving all in-app
					notifications.
				</div>
			)}
		</div>
