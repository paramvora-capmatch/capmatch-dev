...
async function createGoogleCalendarEvent(
	...
	const selectedCalendar = connection.calendar_list.find(
		(cal) => cal.selected
	);
...
async function ensureCalendarWatchIsActive(
	...
	if (connection.watch_channel_id && connection.watch_expiration) {
		...
		if (expiration.getTime() - now.getTime() > 24 * 60 * 60 * 1000) {
			console.log(
				"Calendar watch is still active for connection:",
				connection.id
			);
			return;
		}
	...
	try {
		const { setupCalendarWatch } = await import(
		...
		if (
			errorMessage.includes("webhookUrlNotHttps") ||
			errorMessage.includes("WebHook callback must be HTTPS")
		) {
			console.warn(
				"⚠️  Calendar watch setup skipped: Google Calendar requires HTTPS for webhooks"
			);
			console.warn(
				"   This is expected in local development (http://localhost)"
			);
			console.warn(
			...
			console.warn(
				"   → Calendar invites will still be sent, but responses won't auto-sync"
			);
		} else {
	...
	for (const connection of connections) {
		...
		try {
			if (connection.provider === "google") {
				...
				console.log(
					`Updated Google Calendar event ${eventInfo.eventId}`
				);
			}
		} catch (error) {
			console.error(
...
export async function disconnectCalendar(
	...
	try {
		const { stopCalendarWatch } = await import(
			"@/services/calendarSyncService"
		);
		...
		console.error(
			"Error stopping calendar watch during disconnect:",
			error
		);
