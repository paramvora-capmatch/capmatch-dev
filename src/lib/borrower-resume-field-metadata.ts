// Define the Borrower Resume field metadata configuration
// This mirrors the structure used for Project Resume fields to support
// enhanced features like Autofill sources, tooltips, and rich data.

export const borrowerResumeFieldMetadata: Record<string, FieldMetadata> = {
	// Section 1: Basic Info
	fullLegalName: {
		value: null,
		source: null,
		warnings: [],
		// Additional metadata properties needed for UI rendering logic
		// These are typed via augmentation or loose usage in components
		fieldId: "fullLegalName",
		description:
			"The full legal name of the borrower entity or individual sponsor.",
		backupSource: "Document (Entity Formation)",
		expectedValue: "Text",
		dataType: "Text",
		section: "basic-info",
	} as any,
	primaryEntityName: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "primaryEntityName",
		description: "Name of the primary borrowing entity.",
		backupSource: "Document (Org Chart)",
		expectedValue: "Text",
		dataType: "Text",
		section: "basic-info",
	} as any,
	primaryEntityStructure: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "primaryEntityStructure",
		description: "Legal structure of the borrowing entity (LLC, LP, etc.).",
		backupSource: "User Input",
		expectedValue: "LLC, LP, S-Corp, C-Corp, etc.",
		dataType: "Dropdown",
		section: "basic-info",
	} as any,
	contactEmail: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "contactEmail",
		description: "Primary contact email for the borrower.",
		backupSource: "N/A",
		expectedValue: "Email Address",
		dataType: "Text",
		section: "basic-info",
	} as any,
	contactPhone: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "contactPhone",
		description: "Primary phone number for the borrower.",
		backupSource: "N/A",
		expectedValue: "Phone Number",
		dataType: "Text",
		section: "basic-info",
	} as any,
	contactAddress: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "contactAddress",
		description: "Mailing address for the borrowing entity.",
		backupSource: "Document (Tax Return)",
		expectedValue: "Address String",
		dataType: "Text",
		section: "basic-info",
	} as any,

	// Section 2: Experience
	yearsCREExperienceRange: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "yearsCREExperienceRange",
		description: "Number of years the sponsor has been active in CRE.",
		backupSource: "User Input",
		expectedValue: "0-2, 3-5, 6-10, 11-15, 16+",
		dataType: "Dropdown",
		section: "experience",
	} as any,
	assetClassesExperience: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "assetClassesExperience",
		description: "Types of assets the sponsor has developed or managed.",
		backupSource: "User Input",
		expectedValue: "Multi-select (Multifamily, Office, etc.)",
		dataType: "Multi-select",
		section: "experience",
	} as any,
	geographicMarketsExperience: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "geographicMarketsExperience",
		description: "Regions where the sponsor has experience.",
		backupSource: "User Input",
		expectedValue: "Multi-select (Northeast, Southwest, etc.)",
		dataType: "Multi-select",
		section: "experience",
	} as any,
	totalDealValueClosedRange: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "totalDealValueClosedRange",
		description: "Total value of real estate transactions closed to date.",
		backupSource: "User Input",
		expectedValue: "Range string",
		dataType: "Dropdown",
		section: "experience",
	} as any,
	existingLenderRelationships: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "existingLenderRelationships",
		description:
			"Banks or institutions the borrower has worked with previously.",
		backupSource: "Document (REO Schedule)",
		expectedValue: "Text",
		dataType: "Text",
		section: "experience",
	} as any,
	bioNarrative: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "bioNarrative",
		description: "Short biography of the sponsor/company.",
		backupSource: "User Input",
		expectedValue: "Paragraph text",
		dataType: "Textarea",
		section: "experience",
	} as any,
	trackRecord: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "trackRecord",
		description: "Historical record of completed projects including project name, year, units, IRR, market, and asset type.",
		backupSource: "Document (Track Record Spreadsheet)",
		expectedValue: "Array of project objects",
		dataType: "Table",
		section: "experience",
	} as any,
	yearFounded: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "yearFounded",
		description: "Year the sponsor entity was founded.",
		backupSource: "Document (Company Profile / Website)",
		expectedValue: "Integer, e.g., 2008",
		dataType: "Integer",
		section: "experience",
	} as any,
	activeProjects: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "activeProjects",
		description: "Number of currently active development projects.",
		backupSource: "Document (Project Pipeline / Track Record)",
		expectedValue: "Integer, e.g., 5",
		dataType: "Integer",
		section: "experience",
	} as any,

	// Section 3: Financial Info
	creditScoreRange: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "creditScoreRange",
		description: "Estimated credit score range of the key guarantor.",
		backupSource: "Document (Credit Report)",
		expectedValue: "Range string",
		dataType: "Dropdown",
		section: "borrower-financials",
	} as any,
	netWorthRange: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "netWorthRange",
		description: "Estimated net worth range of the key guarantor.",
		backupSource: "User Input",
		expectedValue: "Range string",
		dataType: "Dropdown",
		section: "borrower-financials",
	} as any,
	liquidityRange: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "liquidityRange",
		description: "Estimated liquid assets available.",
		backupSource: "User Input",
		expectedValue: "Range string",
		dataType: "Dropdown",
		section: "borrower-financials",
	} as any,
	developmentGuarantees: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "developmentGuarantees",
		description: "Contingent liabilities related to development agreements.",
		backupSource: "Document (Development Agreement)",
		expectedValue: "Text",
		dataType: "Text",
		section: "borrower-financials",
	} as any,
	equitypartnersList: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "equitypartnersList",
		description: "List of equity partners in the sponsor entity.",
		backupSource: "Document (Balance Sheet)",
		expectedValue: "Text",
		dataType: "Text",
		section: "borrower-financials",
	} as any,
	historicalEquityDistributions: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "historicalEquityDistributions",
		description: "Cash paid out to partners.",
		backupSource: "Document (Balance Sheet)",
		expectedValue: "Currency",
		dataType: "Currency",
		section: "borrower-financials",
	} as any,
	retainedEarningsCalculated: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "retainedEarningsCalculated",
		description: "Calculated retained earnings.",
		backupSource: "Document (Balance Sheet)",
		expectedValue: "Currency",
		dataType: "Currency",
		section: "borrower-financials",
	} as any,
	priorYearsRetainedEarnings: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "priorYearsRetainedEarnings",
		description: "Retained earnings from prior years.",
		backupSource: "Document (Balance Sheet)",
		expectedValue: "Currency",
		dataType: "Currency",
		section: "borrower-financials",
	} as any,
	bankruptcyHistory: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "bankruptcyHistory",
		description: "History of bankruptcy in the last 7 years.",
		backupSource: "Document (Background Check)",
		expectedValue: "Yes/No",
		dataType: "Boolean",
		section: "borrower-financials",
	} as any,
	foreclosureHistory: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "foreclosureHistory",
		description: "History of foreclosure in the last 7 years.",
		backupSource: "Document (Background Check)",
		expectedValue: "Yes/No",
		dataType: "Boolean",
		section: "borrower-financials",
	} as any,
	litigationHistory: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "litigationHistory",
		description: "Current or past significant litigation.",
		backupSource: "Document (Background Check)",
		expectedValue: "Yes/No",
		dataType: "Boolean",
		section: "borrower-financials",
	} as any,
	scheduleOfRealEstateOwned: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "scheduleOfRealEstateOwned",
		description: "Schedule of Real Estate Owned (SREO) detailing other properties owned.",
		backupSource: "Document (SREO)",
		expectedValue: "Array of Property objects",
		dataType: "Table",
		section: "borrower-financials",
	} as any,
	historicalCostBasis: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "historicalCostBasis",
		description: "Original acquisition cost of past projects.",
		backupSource: "Document (Project Overview)",
		expectedValue: "Currency",
		dataType: "Currency",
		section: "experience",
	} as any,
	historicalExitValues: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "historicalExitValues",
		description: "Realized or appraised exit values of past projects.",
		backupSource: "Document (Project Overview)",
		expectedValue: "Currency",
		dataType: "Currency",
		section: "experience",
	} as any,
	constructionWorkInProgress: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "constructionWorkInProgress",
		description: "Value of CIP asset.",
		backupSource: "Document (Balance Sheet)",
		expectedValue: "Currency",
		dataType: "Currency",
		section: "experience",
	} as any,

	// Section 4: Online Presence
	linkedinUrl: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "linkedinUrl",
		description: "Link to LinkedIn profile.",
		backupSource: "N/A",
		expectedValue: "URL",
		dataType: "Text",
		section: "online-presence",
	} as any,
	websiteUrl: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "websiteUrl",
		description: "Link to company website.",
		backupSource: "N/A",
		expectedValue: "URL",
		dataType: "Text",
		section: "online-presence",
	} as any,

	// Section 5: Principals (Metadata for individual fields within array items)
	principalLegalName: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "principalLegalName",
		description: "Name of a key principal.",
		backupSource: "Document (Org Chart)",
		expectedValue: "Text",
		dataType: "Text",
		section: "principals",
	} as any,
	principalRoleDefault: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "principalRoleDefault",
		description: "Role of the principal (e.g., Sponsor, Guarantor).",
		backupSource: "Document (Org Chart)",
		expectedValue: "Dropdown",
		dataType: "Dropdown",
		section: "principals",
	} as any,
	principalEmail: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "principalEmail",
		description: "Email of the principal.",
		backupSource: "N/A",
		expectedValue: "Email",
		dataType: "Text",
		section: "principals",
	} as any,
	ownershipPercentage: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "ownershipPercentage",
		description: "Percentage of ownership in the borrowing entity.",
		backupSource: "User Input",
		expectedValue: "Percentage (0-100)",
		dataType: "Percent",
		section: "principals",
	} as any,
	principalBio: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "principalBio",
		description: "Short bio for the principal.",
		backupSource: "User Input",
		expectedValue: "Text",
		dataType: "Textarea",
		section: "principals",
	} as any,
	principalSpecialties: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "principalSpecialties",
		description: "Areas of expertise and specialization for the principal.",
		backupSource: "User Input",
		expectedValue: "Array of strings",
		dataType: "Multi-select",
		section: "principals",
	} as any,
	principalAchievements: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "principalAchievements",
		description: "Key accomplishments and notable achievements of the principal.",
		backupSource: "User Input",
		expectedValue: "Array of strings",
		dataType: "Multi-select",
		section: "principals",
	} as any,
	principalEducation: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "principalEducation",
		description: "Educational background of the principal (degrees, institutions).",
		backupSource: "User Input",
		expectedValue: "Text",
		dataType: "Text",
		section: "principals",
	} as any,

	// Container field for the principals array itself
	principals: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "principals",
		description: "List of key principals and their details.",
		backupSource: "Document (Org Chart)",
		expectedValue: "Array of Principal objects",
		dataType: "Table",
		section: "principals",
	} as any,

	// Section 6: References
	references: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "references",
		description: "List of lender references including firm name, relationship type, years of relationship, and contact information.",
		backupSource: "Document (Reference List)",
		expectedValue: "Array of reference objects",
		dataType: "Table",
		section: "references",
	} as any,
	// SREO Table
	sreoProperties: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "sreoProperties",
		description: "Detailed Schedule of Real Estate Owned (SREO).",
		backupSource: "Document (SREO)",
		expectedValue: "Array of SREOProperty",
		dataType: "Table",
		section: "borrower-financials",
	} as any,
	sreoPropertyName: {
		fieldId: "propertyName",
		description: "Name of the property.",
		expectedValue: "Text",
		dataType: "Text",
		section: "borrower-financials",
	} as any,
	sreoPropertyAddress: {
		fieldId: "propertyAddress",
		description: "Address of the property.",
		expectedValue: "Text",
		dataType: "Text",
		section: "borrower-financials",
	} as any,
	sreoAssetType: {
		fieldId: "assetType",
		description: "Asset type of the property.",
		expectedValue: "Text",
		dataType: "Text",
		section: "borrower-financials",
	} as any,
	sreoCurrentValue: {
		fieldId: "currentValue",
		description: "Current market value of the property.",
		expectedValue: "Currency",
		dataType: "Currency",
		section: "borrower-financials",
	} as any,
	sreoDebtBalance: {
		fieldId: "debtBalance",
		description: "Remaining debt balance on the property.",
		expectedValue: "Currency",
		dataType: "Currency",
		section: "borrower-financials",
	} as any,
	sreoEquity: {
		fieldId: "equity",
		description: "Equity in the property.",
		expectedValue: "Currency",
		dataType: "Currency",
		section: "borrower-financials",
	} as any,
	sreoNoi: {
		fieldId: "noi",
		description: "Net Operating Income of the property.",
		expectedValue: "Currency",
		dataType: "Currency",
		section: "borrower-financials",
	} as any,
	sreoDscr: {
		fieldId: "dscr",
		description: "Debt Service Coverage Ratio of the property.",
		expectedValue: "Decimal",
		dataType: "Decimal",
		section: "borrower-financials",
	} as any,
	// Assets Table
	assets: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "assets",
		description: "Personal or corporate assets.",
		backupSource: "Document (Balance Sheet / PFS)",
		expectedValue: "Array of AssetItem",
		dataType: "Table",
		section: "borrower-financials",
	} as any,
	assetName: {
		fieldId: "name",
		description: "Name/Description of the asset.",
		expectedValue: "Text",
		dataType: "Text",
		section: "borrower-financials",
	} as any,
	assetValue: {
		fieldId: "value",
		description: "Value of the asset.",
		expectedValue: "Currency",
		dataType: "Currency",
		section: "borrower-financials",
	} as any,
	assetNotes: {
		fieldId: "notes",
		description: "Notes about the asset.",
		expectedValue: "Text",
		dataType: "Text",
		section: "borrower-financials",
	} as any,
	// Liabilities Table
	liabilities: {
		value: null,
		source: null,
		warnings: [],
		fieldId: "liabilities",
		description: "Personal or corporate liabilities.",
		backupSource: "Document (Balance Sheet / PFS)",
		expectedValue: "Array of LiabilityItem",
		dataType: "Table",
		section: "borrower-financials",
	} as any,
	liabilityName: {
		fieldId: "name",
		description: "Name/Description of the liability.",
		expectedValue: "Text",
		dataType: "Text",
		section: "borrower-financials",
	} as any,
	liabilityValue: {
		fieldId: "value",
		description: "Value/Balance of the liability.",
		expectedValue: "Currency",
		dataType: "Currency",
		section: "borrower-financials",
	} as any,
	liabilityNotes: {
		fieldId: "notes",
		description: "Notes about the liability.",
		expectedValue: "Text",
		dataType: "Text",
		section: "borrower-financials",
	} as any,
};

// Type guard/helper
export interface FieldMetadata {
	value: any;
	source: string | null;
	sources?: string[]; // Array of all sources
	warnings: string[];
	original_value?: any;

	// Extended UI metadata
	fieldId: string;
	description: string;
	backupSource?: string;
	expectedValue: string;
	dataType?: string;
	section: string;
}