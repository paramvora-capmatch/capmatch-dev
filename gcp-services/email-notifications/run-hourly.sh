#!/usr/bin/env bash
# Email notifications - hourly digest runner
# Usage: ./run-hourly.sh
#
# This script runs the email-notifications hourly job in a Docker container.
# Ensure you have:
...
LOG_FILE="${EMAIL_NOTIFICATIONS_HOURLY_LOG:-/var/log/email-notifications-hourly.log}"
...
"$DOCKER_BIN" run --rm \
  ...
  uv run python main_hourly.py >> "$LOG_FILE" 2>&1
