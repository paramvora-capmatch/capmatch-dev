...
FOR SELECT USING (
  ...
  EXISTS (
    ...
    WHERE pag1.user_id = auth.uid() AND pag2.user_id = profiles.id
  )
  OR
  ...
  EXISTS (
    ...
    WHERE om.user_id = auth.uid() AND om.role = 'owner' -- Current user is owner
  ...
  EXISTS (
    ...
    JOIN public.org_members om ON p.owner_org_id = om.org_id
    JOIN public.project_access_grants pag ON p.id = pag.project_id
    WHERE pag.user_id = auth.uid() -- Current user is grantee
