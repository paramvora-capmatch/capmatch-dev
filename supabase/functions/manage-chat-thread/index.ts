...
serve(async (req) => {
  ...
  try {
    const { action, thread_id, project_id, topic, participant_ids } = await req.json();
    ...
    if (action === "create") {
      ...
      if (!isOwner.data && !isAdvisor) {
        throw new Error("You don't have permission to create threads for this project");
      }
      ...
      if (participant_ids && participant_ids.length > 0) {
        ...
        if (participantsError) {
          console.warn(`Failed to add some participants: ${participantsError.message}`);
      ...
      if (!isOwner.data && !isAdvisor) {
        throw new Error("You don't have permission to add participants to this thread");
      }
      ...
      if (!isOwner.data && !isAdvisor) {
        throw new Error("You don't have permission to remove participants from this thread");
      }
      ...
      if (participantsError) {
        throw new Error(`Failed to remove participants: ${participantsError.message}`);
      }
