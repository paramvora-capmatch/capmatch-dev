...
const EnhancedProjectForm: React.FC<EnhancedProjectFormProps> = ({
	...
	const isFieldAutofilled = useCallback(
		(fieldId: string) => {
			...
			const isUserInput = meta.sources.some((src: any) => {
				if (typeof src === 'string') return src.toLowerCase() === 'user_input' || src.toLowerCase() === 'user input';
			...
			return (
				<div className="mb-1">
					<label className="flex text-sm font-medium text-gray-700 items-center gap-2 relative group/field w-full">
						...
						<FieldHelpTooltip fieldId={fieldId} fieldMetadata={fieldMetadata[fieldId]} />
						{warning && (
	...
	useEffect(() => {
		...
		const newLockedFields = new Set(
			Object.keys(existingProject._lockedFields || {}).filter(k => existingProject._lockedFields?.[k])
		);
		
		Object.entries(metadata).forEach(([fieldId, meta]) => {
			// Check if source is AI/Document
			const isAiSourced = meta.sources && meta.sources.some((src: any) => {
				if (typeof src === 'string') return src.toLowerCase() !== 'user_input' && src.toLowerCase() !== 'user input';
				return src.type !== 'user_input';
			});

			const hasValue = isProjectValueProvided((existingProject as any)[fieldId]);
			...
			<div className="p-4">
				{showAutofillNotification && (
					<div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-md text-xs text-blue-800 flex gap-2">
						...
						<span>
							Autofill complete. Data from documents has been applied.
						</span>
					</div>
				)}
				<FormWizard
